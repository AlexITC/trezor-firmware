...
/// def toif_info(self, image: bytes) -> (int, int, bool):
///     """
...
STATIC mp_obj_t mod_trezorui_Display_image(size_t n_args,
  ...
  uint16_t h = 0;
  bool grayscale = false;
  bool valid = display_toif_info(image.buf, image.len, &w, &h, &grayscale);
  if (!valid || grayscale) {
    mp_raise_ValueError("Invalid image format");
  }
  display_image(x, y, w, h, image.buf + 12, image.len - 12);
  return mp_const_none;
}
...
STATIC mp_obj_t mod_trezorui_Display_avatar(size_t n_args,
  ...
  display_avatar(x, y, image.buf + 12, image.len - 12, fgcolor, bgcolor);
  return mp_const_none;
}
...
STATIC mp_obj_t mod_trezorui_Display_icon(size_t n_args, const mp_obj_t *args) {
  ...
  uint16_t h = 0;
  bool grayscale = false;
  bool valid = display_toif_info(icon.buf, icon.len, &w, &h, &grayscale);
  if (!valid || !grayscale) {
    mp_raise_ValueError("Invalid image format");
  }
  mp_int_t fgcolor = mp_obj_get_int(args[4]);
  mp_int_t bgcolor = mp_obj_get_int(args[5]);
  display_icon(x, y, w, h, icon.buf + 12, icon.len - 12, fgcolor, bgcolor);
  return mp_const_none;
}
